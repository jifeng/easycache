...
Cache.prototype._initTair = function (cb) {
  var options = this.options;
  var self = this;
  var tair = Tair(options);
  tair.on('error',function (err) {
    return cb && cb(err);
  });
  tair.once('connect', function () {
    console.log('tair server connected');
    return cb && cb();
  });
  // 封装 tair api 为 redis 的接口
  this.storage = {
    get: tair.get,
    setex: function (key, expire, value, callback) {
      // expired 秒级别，默认24小时
      tair.put(key, value, {expired: expire}, callback);
    },
    del: function (keys, callback) {
      ...
      var func = function () {
        ...
        tair.put(key, null, {expire: 1}, this);
      };
      var tasks = [];
      ...
      console.log('tair client not support end');
      return callback && callback(null);
    },
    flushdb: function (callback) {
      console.log('tair client not support flushdb');
      return callback && callback(null);
    },
    smembers: function (callback) {
      console.log('tair client not support smembers');
      return callback && callback(null);
    },
    sadd: function (callback) {
      console.log('tair client not support sadd');
      return callback && callback(null);
    },
    srem: function (callback) {
      console.log('tair client not support srem');
      return callback && callback(null);
    }
  };
}
